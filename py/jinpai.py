import json,sys,threading,uuid,requests,time
sys.path.append('..')
...
class Spider(Spider):
    ...
    def homeContent(self,filter):
        ...
        filters={}
        for k in cdata['data']:
            classes.append({'type_name':k['typeName'],'type_id':str(k['typeId']),})
        ...
        for tid,d in fdata['data'].items():
            ...
            filters[tid] = [
                {"key": "type", "name": "类型", "value": [{"n": i["itemText"], "v": i["itemValue"]} for i in d["typeList"]]},
                *([] if not d["plotList"] else [{"key": "v_class", "name": "剧情", "value": [{"n": i["itemText"], "v": i["itemText"]} for i in d["plotList"]]}]),
                {"key": "area", "name": "地区", "value": [{"n": i["itemText"], "v": i["itemText"]} for i in d["districtList"]]},
                {"key": "year", "name": "年份", "value": [{"n": i["itemText"], "v": i["itemText"]} for i in d["yearList"]]},
                {"key": "lang", "name": "语言", "value": [{"n": i["itemText"], "v": i["itemText"]} for i in d["languageList"]]},
    ...
    def categoryContent(self,tid,pg,filter,extend):
        params={"area":extend.get('area',''),"filterStatus":"1","lang":extend.get('lang',''),"pageNum":pg,"pageSize":"30","sort":extend.get('sort','1'),"sortBy":"1","type":extend.get('type',''),"type1":tid,"v_class":extend.get('v_class',''),"year":extend.get('year','')}
    ...
    def detailContent(self,ids):
        ...
        vod['vod_play_from']='🏅jinpai🏅'
        vod['vod_area']='🌏' + vod['vod_area'] + '🌏'
        vod['vod_actor']='🎯' + vod['vod_actor'] + '🎯'
        vod['vod_director']='⚡️' + vod['vod_director'] + '⚡️'
        vod['vod_content']='🔥' + vod['vod_content'] + '🔥'
        vod['vod_play_url']='#'.join(f"{i['name'] if len(vod['episodelist']) > 1 else vod['vod_name']}${ids[0]}@@{i['nid']}" for i in vod['episodelist'])
        vod.pop('episodelist',None)
        return {'list':[vod]}

    def searchContent(self,key,quick,pg="1"):
        params={"keyword":key,"pageNum":pg,"pageSize":"8","sourceCode":"1"}
    ...
    def playerContent(self,flag,id,vipFlags):
        self.header={'User-Agent':'okhttp/5.1.0','sec-ch-ua-platform':'"Windows"','DNT':'1','sec-ch-ua':'"Not/A)Brand";v="8","Chromium";v="126","Google Chrome";v="126"','sec-ch-ua-mobile':'?0','Origin':self.host,'Referer':f'{self.host}/'}
        ids=id.split('@@')
    ...
    def getheaders(self,param=None):
        ...
        headers={'User-Agent':'okhttp/5.1.0','Accept':'application/json,text/plain,*/*','sign':sign,'t':t,'deviceid':deviceid}
        return headers
